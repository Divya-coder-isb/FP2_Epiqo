...
def forecast(date, today):
    if (date - today).days < 90:
        ...
        future_dates = pd.date_range(start=df.index[-1] + timedelta(days=1), periods=seq_length, freq='B')
        data_with_future = pd.concat([df, pd.DataFrame({'Close': [np.nan]*seq_length}, index=future_dates)])
        data_with_future['Close'] = data_with_future['Close'].interpolate()
        scaled_data = lstm_scaler.transform(data_with_future[['Close']])
...
def classify_trend(date):
    ...
    st.write(f"RSI for date {date}: {rsi}")  # Debug: Log RSI value
    rsi_scaled = svm_scaler.transform([[rsi]])
    st.write(f"Scaled RSI for date {date}: {rsi_scaled}")  # Debug: Log scaled RSI
    trend = svm_model.predict(rsi_scaled)[0]
    st.write(f"Predicted trend for date {date}: {trend}")  # Debug: Log predicted trend
    return trend
