...
def predict_trend_with_rsi(forecasted_price):
    ...
    data = data.append(pd.DataFrame({'Close': [forecasted_price]}, index=[datetime.today()]))
    ...
    X_scaled = scaler.transform(data[[f'lag_{i}' for i in range(1, 11)]].iloc[-1].values.reshape(1, -1))
...
if st.button("Apply"):
    if validate_date(selected_date):
        ...
        trend, trend_pred = predict_trend_with_rsi(price)
