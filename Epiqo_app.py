...
Original file is located at
    https://colab.research.google.com/drive/1hZI4TfhTJ7nT7A9Xtf8NbmDsCkeKeND_
"""
...
def create_features(date):
    ...
    # Add the lag and rolling mean/std features
    df_with_date = df.append(temp_df).sort_index()
    temp_df['lag_1'] = df_with_date['Close'].shift(1).iloc[-1]
    temp_df['lag_2'] = df_with_date['Close'].shift(2).iloc[-1]
    temp_df['lag_3'] = df_with_date['Close'].shift(3).iloc[-1]
    temp_df['rolling_mean_3'] = df_with_date['Close'].rolling(window=3).mean().iloc[-1]
    temp_df['rolling_std_3'] = df_with_date['Close'].rolling(window=3).std().iloc[-1]
    temp_df['rolling_mean_7'] = df_with_date['Close'].rolling(window=7).mean().iloc[-1]
    temp_df['rolling_std_7'] = df_with_date['Close'].rolling(window=7).std().iloc[-1]
