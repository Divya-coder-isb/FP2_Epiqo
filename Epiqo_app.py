...
def forecast(date, today):
    if (date - today).days < 90:
        ...
        # Ensure only the last seq_length days are used for LSTM prediction
        recent_data = combined_df.iloc[-seq_length:]

        # Debug: Log the combined_df before scaling
        st.write("Combined DataFrame before scaling:", combined_df.tail(70))

        scaled_data = lstm_scaler.transform(recent_data[['Close']])
