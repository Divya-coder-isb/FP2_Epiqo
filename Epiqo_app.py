...
def predict_trend(selected_date):
    features, data = prepare_data_for_prediction(selected_date)
    ...
    # Predict trend using SVM
    X_scaled = scaler.transform(data[[f'lag_{i}' for i in range(1, num_lags + 1)]].iloc[-1].values.reshape(1, -1))
    trend_pred = svm_model.predict(X_scaled)[0]
...
if st.button("Apply"):
    if validate_date(selected_date):
        ...
        trend, trend_pred = predict_trend(selected_date)
        st.write(f"Market Trend: {trend} ({trend_pred})")
        if trend_image:
